...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (message.author.id != "846794001050894360")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only Emmaa is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/2WHg3Qs6AK), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a::Cargando_DoS:891783323339358229> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
