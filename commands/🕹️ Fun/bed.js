...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
     ...
     if(!user1 && args[0] && args[0].length == 18) {
     ...
     else if(!user1 && args[0]){
       let alluser = message.guild.members.cache.map(member=> String(member.user.username).toLowerCase())
       user1 = alluser.find(user => user.includes(args[0].toLowerCase()))
       user1 = message.guild.members.cache.find(me => (me.user.username).toLowerCase() == user1).user
       if(!user1 || user1 == null || !user1.id) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["bed"]["variable3"])})
     }
     else {
       user1 = message.mentions.users.first() || message.author;
     }
     //find the USER
     ...
     if(user1.id == user2.id) user2 == message.author;
     ...
     let avatar1 = user1.displayAvatarURL({
       dynamic: false,
       format: "png"
     });
    ...
    client.memer.bed(avatar1, avatar2).then(image => {
      ...
      message.reply({embeds : [tempmsg.embeds[0]
        .setAuthor(`Meme for: ${user1.tag} `, avatar1)
        .setColor(es.color)          
